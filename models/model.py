...
def cornetv2(audio_feature_shape, noise_label_feature_shape, save_path=None):
    """
    mel频谱和midi之间的lag预测，不支持变长输出。输入序列长度320帧（约10s），容忍mel和lag间最大延时??,
    模型输出的lab为0～1之间的浮点值，代表lag / 320
    :param audio_feature_shape:
    :param noise_label_feature_shape:
    :param save_path:
    ...
    def siamese_feature(feature):
        x = feature
        # 第一阶段
        stage0 = stack1(x, 256, 3, stride1=(1, 2), name='conv5')  # l/16, 8
        stage0_output = MaxPooling2D((1, 8), strides=(1, 1))(stage0)  # 全局池化

        # 第二阶段
        stage1 = stack1(stage0, 256, 3, stride1=(1, 1), name='conv6')  # l/16, 8
        stage1_output = MaxPooling2D((1, 8), strides=(1, 1))(stage1)  # 全局池化

        # 第三阶段
        stage2 = stack1(stage1, 256, 3, stride1=(1, 1), name='conv7')  # l/16, 8
        stage2_output = MaxPooling2D((1, 8), strides=(1, 1))(stage2)  # 全局池化

        return stage0_output, stage1_output, stage2_output
    ...
    af_s0, af_s1, af_s2 = siamese_model(af)
    lf_s0, lf_s1, lf_s2 = siamese_model(lf)

    # model = Model(inputs=[audio_feature_input, label_feature_input], outputs=[af_s, lf_s],
    #               name=f"light_crnn")

    corf0 = cor_block(af_s0, lf_s0)
    corf1 = cor_block(af_s1, lf_s1)
    corf2 = cor_block(af_s2, lf_s2)

    corf_final = Add()([corf0, corf1, corf2])

    # model = Model(inputs=[audio_feature_input, label_feature_input], outputs=[corf],
    #               name=f"light_crnn")
    #
    reg = regression(corf_final)  # 回归头
...
if __name__ == '__main__':
    ...
    # v2
    pretrain_model = "/data/projects/LabelModels/lag_prediction/train_output_models/cornetv2.h5"
    model = cornetv2(
        audio_feature_shape=(320, 229, 1),
        noise_label_feature_shape=(320, 88, 1),
        save_path=pretrain_model,
